...
- [ ] PowerShell transcript attached to the PR or issue.
- [ ] Environment inventory included in the PR or issue.
...
Evidence files generated by local runs are transient review artifacts. Keep them
under `.pi/evidence/` or another ignored local directory and attach them to the
PR/discussion when needed; do not commit screenshots, recordings, transcripts,
or machine inventory files to the distribution source tree.
